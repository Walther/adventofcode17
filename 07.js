...
// Sorry for the mess of having the inputs here.
// Might clean up these to separate files later on...
const test = `pbga (66)
xhth (57)
ebii (61)
havc (66)
...
const treeParser = input => {
    // String parsing
    let rows = input.split('\n');
    let leaves = rows.filter(row => !row.includes('->'));
    ...
    let nodes = rows.filter(row => row.includes('->'));
    nodes = nodes.map(node => node.split(' -> '));
    nodes = nodes.map(([node, children]) => ({
        name: node.split(' ')[0],
        value: Number(
            node
                .split(' ')[1]
